...
async def start(message: types.Message, state: FSMContext):
    ...
    markup.add(
        types.InlineKeyboardButton("Start convertion", callback_data="start")
    )
    markup.add(
        types.InlineKeyboardButton("Change language", callback_data="language")
    )
...
async def choose_language(message: types.Message):
    markup = types.InlineKeyboardMarkup()
    logger.debug("Set language command handle")
    for code, language in i18n.AVAILABLE_LANGUAGES.items():
        markup.add(
            types.InlineKeyboardButton(
                text=language.label,
                callback_data=chat_settings.new(
                    id=message.from_id, property="language", value=code
                ),
            )
        )
...
@dp.callback_query_handler(
    chat_settings.filter(property="language"), state="*"
)
async def set_language(
    callback_query: types.CallbackQuery, callback_data: dict
):
    logger.info(
        f"Set language recieved: {callback_data}"
        f"user: {callback_query.from_user}"
    )
    ...
    await bot.send_message(
        ...
        text=_("Language is changed to " "{locale_name}").format(
            locale_name=i18n.AVAILABLE_LANGUAGES[locale].label
        ),
    )


@dp.callback_query_handler(
    lambda c: c.data and c.data.endswith("measure"),
    state=Convert.choosing_measure_type,
)
async def process_measure_type_choosen(
    callback_query: types.CallbackQuery, state: FSMContext
):
    pressed_button = keyboards.get_pressed_button(
        main_keyboard_buttons, callback_query.data
    )
...
@dp.message_handler(
    lambda message: is_float(message.text), state=Convert.set_measure_value
)
async def process_measure_value(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(
        message.chat.id,
        _("Specify measure units please."),
...
@dp.message_handler(
    lambda message: not is_float(message.text), state=Convert.set_measure_value
)
...
@dp.callback_query_handler(
    lambda c: c.data and c.data.endswith("_units"),
...
async def process_measure_eng_units_choosen(
    ...
    pressed_button = keyboards.get_pressed_eu_button(
        measure_class, callback_query.data
    )
...
async def process_physical_measure_scale_low(
...
async def process_physical_measure_scale_hi(
    message: types.Message, state: FSMContext
):
...
async def process_physical_measure_eu(
    message: types.Message, state: FSMContext
):
    physical_measure_eu = message.text

    await state.update_data(physical_measure_eu=physical_measure_eu)
    ...
    await bot.send_message(
        ...
        reply_markup=create_eu_keyboard_for_measure(selected_measure_class),
    )
...
async def process_convertion_units_choosen(
    callback_query: types.CallbackQuery, state: FSMContext
):
    user_data = await state.get_data()
    measure_class = user_data["measure_class"]
    ...
    converted_measure: Measure = current_measure.convert_to(new_measure_eu)

    await bot.send_message(
        callback_query.from_user.id,
        _("{measure}" "\n is equal to\n" "{converted}").format(
            measure=hbold(str(current_measure)),
